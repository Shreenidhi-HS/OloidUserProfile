                ...
                openCamera ? (
                    <>
                        {!src &&
                            <>
                             ...
                             <div className='absolute rounded-[0.75rem] md:fixed top-0 left-0 h-screen w-full z-[999] bg-[black] overflow-hidden md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-1/2 md:h-3/4'>
                                <div className='bg-white z-[999] relative p-3 w-[2rem] h-[2rem] flex items-center justify-center float-right mr-3 mt-3 rounded-[0.45rem] cursor-pointer' onClick={handleCameraClose}>X</div>
                                <Webcam
                                    audio={false}
                                    height={'50%'}
                                    ref={webcamRef}
                                ...
                                />
                                {/* capture button */}
                                <div className='cursor-pointer z-[999] hover:scale-90 transition-all duration-300 bg-white p-2 rounded-full absolute bottom-0 left-1/2 -translate-x-1/2 mb-16 w-[4rem] h-[4rem]' 
                                onClick={(e) =>{
                                    e.preventDefault();
                                    capture();
                                } }>
                                    <div className='bg=white w-[3rem] h-[3rem] border-[3px] border-[black] rounded-full'>
                                    </div>
                                </div>
                            </div>
                            </>
                        ...
                        <div className='absolute rounded-[0.75rem] md:fixed top-0 left-0 h-screen w-full z-[999] bg-[#272727] overflow-hidden md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-1/2 md:h-3/4'>
                             <div className='bg-white p-2 w-[2rem] h-[2rem] flex items-center justify-center float-left ml-3 mt-3 rounded-[0.45rem] cursor-pointer' onClick={handleCameraClose}><img src="../../public/assets/credentials/back-btm.svg" height={30} width={30} /></div>
                             <h2 className='text-white text-2xl font-bold absolute top-0 left-1/2 -translate-x-1/2 mt-[15%] md:mt-[5%]'>Selfie</h2>
                            <div className='bg-white rounded-t-[2.75rem] p-8 flex flex-col gap-4 items-center h-full mt-32'>
                                <div className='w-[200px] h-[200px] rounded-[0.75rem] border-[1px] border-[#D0D5DD] overflow-hidden'>
                                    <img src={src} alt={`Captured image`} className='h-full w-full' />
                                </div>
                                <h2 className='text-xl font-medium'>Add Credential</h2>
                                <span className='text-center'>Please make sure your face is not blurred or out of frame before you continue</span>
                                <button 
                                    onClick={(e) => saveCapture(e)} 
                                    className='py-4 min-w-[20rem] text-white text-lg bg-[#272727] rounded-[0.75rem] disabled:bg-gray-200'>
                                   {addFaceMutate.isLoading ? <img src="../../public/assets/loader.svg" className=" animate-spin m-auto" height={24} width={24} alt="loader" /> : "Looks Great! Continue"} 
                                </button>
                                <button onClick={retakePhoto} className='py-4 min-w-[20rem] border-[1px] border-[#272727] text-[#272727] text-lg bg-white rounded-[0.75rem]'>Take a new photo</button>
                            </div>
                         </div>
                        </>
                    ...
                    (
                        <div className='grid grid-cols-3 md:grid-cols-5 gap-2'>
                            <div className="flex items-center justify-center w-full bg-white">
                                <label className="cursor-pointer flex flex-col w-full h-32 border-2 border-dashed border-[#82B4E5] hover:border-[#475467] transition-all duration-300 rounded-[0.75rem]" onClick={handleCaptureImage}>
                                    <div className="flex flex-col items-center justify-center pt-7">
                                        <svg width="34" height="34" viewBox="0 0 61 61" fill="#475467" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M10.8555 30.2651H50.8555M30.8555 10.2651V50.2651" stroke="#475467" stroke-width="5" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                        <p className="pt-1 text-sm tracking-wider text-[#475467] group-hover:text-gray-600">Add Face</p>
                                    </div>
                                </label>
                            </div>
                            ...
                            <>
                              {userData?.data.user.Faces.map((data, index) => (
                                <div className='h-32 relative rounded-[0.75rem] overflow-hidden w-fit' key={index}>
                                    <input
                                        type="checkbox"
                                        name="delete"
                                        id={`delete-${index}`}
                                        className='absolute top-0 left-0 ml-2 mt-2 rounded-full cursor-pointer'
                                        onChange={() => toggleImageSelection(data.OnlineModelFaceID)}
                                        checked={selectedImages.includes(data.OnlineModelFaceID)}
                                    />
                                    <img src={data.SignedUrl} alt={`Captured ${index + 1}`} className='h-full rounded-[0.75rem]' />
                                </div>
                            ))}
                            </>}
                ...
                <DialogContent className='max-w-[90vw] md:max-w-md'>
                    <DialogHeader>
                        ...
                        <DialogDescription className='p-3'>
                            <span>Are you sure want to delete the selected Face credentials!</span>
                            <div className='flex justify-between gap-4 mt-4'>
                                <Button className='bg-transparent flex-1 text-black border-[black] border-[1px]' onClick={toggledeleteModal}>Cancel</Button>
                                <Button className='bg-[red] flex-1' onClick={handleDeleteImages}>{removeFaceMutate.isLoading ? <img src="../../public/assets/loader.svg" className=" animate-spin m-auto" height={24} width={24} alt="loader" /> : "Delete"}</Button>
                            </div>
                        </DialogDescription>
                    </DialogHeader>
                </DialogContent>
